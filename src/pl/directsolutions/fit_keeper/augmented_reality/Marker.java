...
public class Marker
{
	...
	void cCMarker(MixVector originalPoint, Camera viewCam, float addX, float addY)
	{
	...
	void calcV(Camera viewCam)
	{
		...
		if (cMarker.z < -1f)
		{
			...
			if (MixUtils.pointInside(cMarker.x, cMarker.y, 0, 0, viewCam.width, viewCam.height))
			{
	...
	void draw(PaintScreen dw)
	{
		...
		if (textBlock == null)
		{
			textBlock = new TextObj(mText+" "+new Double(distance).intValue()/*df.format(distance)*/+"m", Math.round(maxHeight / 2f) + 1, 160, dw);
		}
		...
		if (isVisible)
		{
			//default color
			dw.setColor(color);
			dw.setColor(Color.rgb(0, 204, 255));
