...
public final class RSS14Reader extends OneDReader {
  ...
  private Pair decodePair(BitArray row, boolean right, int rowNumber, Hashtable hints) {
    try {
      ...
      FinderPattern2 pattern = parseFoundFinderPattern(row, rowNumber, right, startEnd);
  ...
  private DataCharacter decodeDataCharacter(BitArray row, FinderPattern2 pattern, boolean outsideChar)
  ...
  private FinderPattern2 parseFoundFinderPattern(BitArray row, int rowNumber, boolean right, int[] startEnd)
      throws NotFoundException {
    ...
    return new FinderPattern2(value, new int[] {firstElementStart, startEnd[1]}, start, end, rowNumber);
  }
